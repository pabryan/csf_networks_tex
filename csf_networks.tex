...
\begin{proof}
Equation \eqref{eq:comparison_ineq} from the Comparison Theorem \ref{thm:comparison} will be satisfied if we can show that $\varphi_c$ is concave and that
\begin{equation}
...
\end{equation}
since the hypothesies of the theorem give
\[
...
\]
noting that concavity of $\varphi_c$, $\varphi_c(0) = 0$, the symmetry of $\varpphi_c$ about $x=\pi$, and the non-negativity of $\varphi_c$ imply that for $x \in [0, \pi]$, $\varphi_c' \geq 0$ and $(\varphi_c - x \varphi_c') \geq 0$. The latter follows by observing that the function $f(x) = \varphi_c(x) - x \varphi_c'(x)$ satisfies $f(0) = 0$ and $f'(x) = -x \varphi_c''(x) \geq 0$.
...
\end{split}
\]
On the self-similar solution $\psi_0$ satisfies
\[
...
Next, by taking a Taylor expansion around $\psi_0'$ with second order remainder, the concavity of $f_0$ implies the remainder is non-negative and hence
\[
f_0(\Phi_c' \psi_0') - f_0(\psi_0') \geq f_0'(\psi_0')(\Phi_c'\psi_0' - \psi_0') = \psi_0' f_0'(\psi_0')(\Phi_c' - 1).
\]
Let us also write $u = e^{-\tau} \psi_0$ for the argument to $\Phi_c$ so that equation \eqref{eq:Phi} becomes
\[
0 \leq 4 (e^{-\tau} \psi_0')^2 \Phi_c'' + \left(\frac{L}{4\pi^2} \bar{Q}_0 - 1\right) \left(\Phi_c - u \Phi_c'\right) + \frac{1}{x} \psi_0' e^{-\tau} \psi_0' f_0'(\psi_0')(\Phi_c' - 1) \Phi_c'.
\]
\end{proof}
...
\]
The self-similar solution is the circle, hence the curvature is constant and we have equality in \holder{}'s inequality yeilding,
\[
\frac{L}{4\pi^2} \bar{Q}_0 = 1, \quad \frac{L}{2\pi} \Delta Q_0 = \frac{2\pi}{L} \ell = x = \frac{4}{x} \arccos^2(\psi_0')
\]
since $\psi_0'(x) = \cos(x/2)$. Thus $f_0 = 4 \arccos^2$.

In this setting, equation \eqref{eq:Phi} becomes
\[
0 \leq 4 (e^{-\tau} \psi_0')^2 \Phi_c'' + \frac{1}{x} \left(f_0(\Phi_c' \psi_0') - f_0(\psi_0')\right) \psi_0' e^{-\tau} \Phi_c'.
\]
\end{remark}
