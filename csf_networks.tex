...
Next, let us note that for strictly convex \(\gamma\), there can be at most finitely many non-regular optimal points with such points only occurring as pairs of nodes.

\begin{lemma}
\label{lem:convex_optimum_regular_points}

Let \(\gamma\) be strictly convex. Then all but at most finitely many \(x_0\) are regular points. More precisely, let \((p_0,q_0) \in \sphere^1 \times \sphere^1\) be optimal; that is \(Z(\tfrac{2\pi}{L}\ell(p_0,q_0)) = \tfrac{L}{2\pi} d(p_0, q_0)\). Then either \((p_0, q_0) \in \gamma^{\ast} \times \gamma^{\ast}\) or \((p_0, q_0) = (a_i, a_j)\) for some \(i \ne j\).
\end{lemma}

\begin{proof}

Let us offer two proofs, both of which rule out the possibility that one of $p_0, q_0$ is a node while the other is not. Then optimum points must either be regular or contained in the finite set \(\{x_{ij} = \tfrac{L}{2\pi} \ell(a_i, a_j)\}_{i\ne j} \subset \sphere^1 \times \sphere^1\). The argument is symmetric in \(p_0, q_0\) so without loss of generality, suppose \(p_0 = a_i\) is a node and \(q_0 \in \gamma^{\ast}\) is a smooth point.

The first method is simple, using basic facts about piecewise smooth, convex curves but which cannot be generalised to the non-convex setting. The second method is variational and gives a criteria at the nodes for when similar conclusions as in the lemma may be deduced for non-convex networks.

\emph{Method 1}: If \((p_0, q_0)\) is optimal, from the first variation at optimal points, Proposition \ref{prop:spatial_var}, we have
\[
\cos\theta_p^{\pm} = \ip{\tang_p^{\pm}}{w} = \ip{\tang_q}{w} = \cos\theta_q \Rightarrow \theta_p^+ = \theta_p^- = \theta_q.
\]
Since \(\tang_p^+ \ne \tang_q^-\) we cannot have \(\theta_p^+ = \theta_p^- = 0\). But for each \(\theta \ne 0\), there are precisely two vectors making angle \(\theta\) with \(w\) and so we conclude that \(\tang_p^+ = \tang_q\) or \(\tang_p^- = \tang_q\). But for a closed, convex curve (even with nodes) this can never happen for \(p_0 \ne q_0\) hence we obtain a contradiction.

\emph{Method 2}: With $p_0 = a_i$ and $q \in \gamma^{\ast}$, we show that there is a length preserving variation strictly decreasing \(d\) and hence \((p_0, q_0)\) cannot be optimum. 

Let \(\alpha_{\pm} (u) = (p_0 \pm u, q_0 \pm u)\). Then \(\ell(\alpha_{\pm} (u)) = \ell(p_0, q_0)\) is constant. Lemma \ref{lem:firstvar} gives
\begin{equation}
\label{eq:method2_1}
\left.\partial_u\right|_{u=0} d (\alpha_{\pm} (u)) = \pm \left(-\ip{\tang^{\pm}(p_0)}{w} + \ip{\tang (q_0)}{w}\right).
\end{equation}

If \(-\ip{\tang^+(p_0)}{w} + \ip{\tang (q_0)}{w} < 0\) then we are done so suppose that,
\[
-\ip{\tang^+(p_0)}{w} + \ip{\tang (q_0)}{w} \geq 0.
\]
Then
\begin{equation}
\label{eq:method2_2}
\ip{\tang^-(p_0)}{w} - \ip{\tang (q_0)}{w} \leq \ip{\tang^-(p_0) - \tang^+(p_0)}{w}.
\end{equation}

Let \(K\) denote the closed region enclosed by \(\gamma\), which is a closed, convex set. Since \(p_0, q_0 \in K\) the line joining \(p_0\) to \(q_0\) is contained in \(K\) and hence \(w\) is inward pointing. Write $w = \cos \theta^+ \tang^+(p_0) - \sin \theta^+ \nor^+(p_0)$ and define,
\[
f(\theta) = \ip{\tang^-(p_0) - \tang^+(p_0)}{\cos \theta \tang^+(p_0) - \sin \theta \nor^+(p_0)} = \frac{1}{2} \cos \theta + \frac{\sqrt{3}}{2} \sin\theta
\]
by applying the triple conditions, $\ip{\tang^+}{\tang^-} = \tfrac{1}{2}$ (equation \eqref{eq:triplecondition}) and $\ip{\tang^-}{\nor^+} = \tfrac{\sqrt{3}}{2}$ (equation \eqref{eq:triplerelations}). From equations \eqref{eq:method2_1} and \eqref{eq:method2_2} we then have,
\[
\left.\partial_u\right|_{u=0} d (\alpha_{-} (u)) \leq f(\theta^+).
\]

The condition that $w$ is inward pointing, and the triple condition $\ip{\tang^+}{-\tang^-} = -\tfrac{1}{2}$ (equation \eqref{eq:triplecondition}) implies that $0 \leq \theta^+ \leq \tfrac{2\pi}{3}$. Thus to produce a length preserving, distance decreasing variation, it suffices to show that $f(\theta) < 0$ for $0 \leq \theta \leq \tfrac{2\pi}{3}$.

Observe that,
\[
f(0) = f\left(\tfrac{2\pi}{3}\right) = -\frac{1}{2},
\]
and on $[0, \tfrac{2\pi}{3}]$, $f$ is strictly convex. Thus $f < 0$.
\end{proof}
