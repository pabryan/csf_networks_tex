...
We make one final estimation: by taking a Taylor expansion around $\psi_0'$ with second order remainder, the concavity of $f_0$ implies the remainder is non-negative and hence
\[
f_0(\Phi_c' \psi_0') - f_0(\psi_0') \geq f_0'(\psi_0')(\Phi_c'\psi_0' - \psi_0') = \psi_0' f_0'(\psi_0')(\Phi_c' - 1).
\]
Let us also write $u = e^{-\tau} \psi_0$ for the argument to $\Phi_c$ so that were finally reduce to
\begin{equation}
\label{eq:Phi}
0 \leq 4 (e^{-\tau} \psi_0')^2 \Phi_c'' + \left(\frac{L}{4\pi^2} \bar{Q}_0 - 1\right) \left(\Phi_c - u \Phi_c'\right) + \frac{1}{x} \psi_0' f_0'(\psi_0')(\Phi_c' - 1) \psi_0' e^{-\tau} \Phi_c'.
\end{equation}
\end{proof}
...
\[
\begin{split}
0 &\leq 4 (e^{-\tau} \psi_0')^2 \Phi_c'' - \frac{8}{x} \psi_0' \frac{\arccos(\psi_0')}{\sqrt{1 - (\psi_0')^2}} (\Phi_c' - 1) \psi_0' e^{-\tau} \Phi_c'. \\
&= 4 (e^{-\tau} \cos(x/2))^2 \Phi_c'' - \frac{8}{x} \cos(x/2) \frac{\cos(x/2)}{\sin(x/2)} (\Phi_c' - 1) \cos(x/2) e^{-\tau} \Phi_c'.
\end{split}
\]
\end{remark}
