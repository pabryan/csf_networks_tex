...
\begin{lemma}
Let \(\gamma\) be strictly convex. Then all but at most finitely many \(x_0\) are regular points. More precisely, let \((p_0,q_0) \in \sphere^1 \times \sphere^1\) be optimal; that is \(Z(\tfrac{2\pi}{L}\ell(p_0,q_0)) = \tfrac{L}{2\pi} d(p_0, q_0)\). Then either \((p_0, q_0) \in \gamma^{\ast} \times \gamma^{\ast}\) or \((p_0, q_0) = (a_i, a_j)\) for some \(1 \leq i < j \leq k\).
\end{lemma}

\begin{proof}
We show that if one of \(p_0, q_0\) is a node but not the other, then there is a length preserving variation strictly decreasing \(d\) and hence \((p_0, q_0)\) cannot be optimum. Then since there are only finitely many nodes, the set \(\{x_{ij} = \tfrac{L}{2\pi} \ell(a_i, a_j)\}_{i\ne j}\) is finite and all other possible optimum points are regular.

Let us offer two proofs.

\emph{Method 1}: The argument is symmetric in \(p_0, q_0\) so without loss of generality, suppose \(p_0 = a_i\) is a node and \(q_0 \in \gamma^{\ast}\) is a smooth point. If \((p_0, q_0)\) is optimal, from the first variation at optimal points, Proposition \ref{prop:spatial_var}, we have
\[
\cos\theta_p^{\pm} = \ip{\tang_p^{\pm}}{w} = \ip{\tang_q}{w} = \cos\theta_q \Rightarrow \theta_p^+ = \theta_p^- = \theta_q.
\]
Since \(\tang_p^+ \ne \tang_q^-\) we cannot have \(\theta_p^+ = \theta_p^- = 0\). But for each \(\theta \ne 0\), there are precisely two vectors making angle \(\theta\) with \(w\) and so we conclude that \(\tang_p^+ = \tang_q\) or \(\tang_p^- = \tang_q\). But for a closed, convex curve (even with nodes) this can never happen for \(p \ne q\) hence we obtain a contradiction.

\emph{Method 2}: Again, the argument is symmetric in \(p_0, q_0\). So let \(p_0 = a_i\) and \(q \in \gamma^{\ast}\). Let \(\alpha_{\pm} (u) = (p_0 \pm u, q_0 \pm u)\). Then \(\ell(\alpha_{\pm} (u)) = \ell(p_0, q_0)\) is constant. Lemma \ref{lem:firstvar} gives
\begin{equation}
\label{eq:method2_1}
\left.\partial_u\right|_{u=0} d (\alpha_{\pm} (u)) = \pm \left(-\ip{\tang^{\pm}(p_0)}{w} + \ip{\tang (q_0)}{w}\right).
\end{equation}

If \(-\ip{\tang^+(p_0)}{w} + \ip{\tang (q_0)}{w} < 0\), then we are done so suppose that \(-\ip{\tang^+(p_0)}{w} + \ip{\tang (q_0)}{w} \geq 0\). Then
\begin{equation}
\label{eq:method2_2}
\ip{\tang^-(p_0)}{w} - \ip{\tang (q_0)}{w} \leq \ip{\tang^-(p_0) - \tang^+(p_0)}{w}.
\end{equation}

Let \(K\) denote the closed region enclosed by \(\gamma\), which is a closed, convex set. Since \(p_0, q_0 \in K\) the line joining \(p_0\) to \(q_0\) is contained in \(K\) and hence \(w\) is inward pointing: \(\ip{w}{\nor^+(p_0)}, \ip{w}{\nor^-(p_0)} < 0\). Writing \(w = \ip{w}{\tang^+(p_0)} \tang^+(p_0) + \ip{w}{\nor^+(p_0)} \nor^+(p_0)\) we then have,
\begin{equation}
\label{eq:method2_3}
\begin{split}
\ip{w}{\tang^-(p_0)} &= \ip{\ip{w}{\tang^+(p_0)} \tang^+(p_0) + \ip{w}{\nor^+(p_0)} \nor^+(p_0)}{\tang^-(p_0)} \\
&= \ip{w}{\tang^+(p_0)} \ip{\tang^+(p_0)}{\tang^-(p_0)} + \ip{w}{\nor^+(p_0)} \ip{\nor^+(p_0)}{\tang^-(p_0)} \\
&= \frac{1}{2} \ip{w}{\tang^+(p_0)} + \frac{\sqrt{3}}{2} \ip{w}{\nor^+(p_0)}.
\end{split}
\end{equation}

Putting equations \eqref{eq:method2_2} and \eqref{eq:method2_3} into \eqref{eq:method2_1} produces a length preserving, strictly distance decreasing variation:
\[
\left.\partial_u\right|_{u=0} d (\alpha_{-} (u)) \leq \ip{\tang^-(p_0) - \tang^+(p_0)}{w} \leq \ip{\tang^-(p_0) - \frac{1}{2}\tang^+(p_0)}{w} = \frac{\sqrt{3}}{2} \ip{w}{\nor^+(p_0)} < 0.
\]
\textbf{But this relies on \(\ip{w}{\tang^+(p_0)} \geq 0\) which cannot be guaranteed. If this fails, it may make \(\ip{w}{\tang^-(p_0)}\) sufficiently negative to overcome this lack of positivity.}
\end{proof}
