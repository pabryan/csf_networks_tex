...
Now the difficulty in obtaining a suitable comparison principle is in dealing with the $\bar{Q}$ and $\Delta Q$ terms, especially the $\Delta Q$ term which is effectively unknown. Both terms are dependent on the loop $\gamma$ and so we need some universal estimates that hold for all $\gamma$. Our resulting theorem is as follows:

\begin{theorem}
Let $\phi : [0, 2\pi] \times \RR_{\geq 0} \to \RR$ be a continuous function satisfying:
\begin{itemize}
\item $\phi$ is smooth and positive on $(0, 2\pi) \times \RR_{>0}$,
\item $\phi$ symmetric about $x = \pi$ for every $t$,
\item $\phi(0, t) = \phi(2\pi, t) = 0$ for every $t$,
\item $\phi$ is weakly concave in $x$ for each $t$ ($\phi'' \leq 0)$,
\item $\limsup_{x\to 0} \frac{\phi}{x} \leq \frac{\sqrt{3}}{2}$,
\end{itemize}
...
\end{split}
\]
and $\phi(x, 0) \leq \chordarcprofile(x, 0)$ for all $x \in [0, 2\pi]$, then $\phi(x, t) \leq \chordarcprofile(x, t)$ for all $x, t$.
\end{theorem}
...
\end{split}
\]
where $\cos \theta = \ip{w}{\tang_{p_0}} = \ip{w}{\tang_{q_0}}$ so that $\cos 2 \theta = \ip{\tang_{p_0}}{\tang_{q_0}}$, and $k(p_0, q_0)$ denotes the number of nodes between $p_0$ and $q_0$.
...
\]
so that the assumptions on $\phi$ and the above inequalities give
\begin{equation}
\label{eq:comparison_diff_ineq_leq}
\partial_t \phi \leq F(\phi'', \phi', \phi, x, t).
\end{equation}

Next, owing to a lack of suitable monotonicity of $F$ in the $y$ variable, we must introduce an exponential factor. We claim that for any $\lambda > 0$, the function $e^{-\lambda t} \chordarcprofile$ is a viscosity solution of
\begin{equation}
\label{eq:exponential_viscosity}
\partial_t \chordarcprofile \geq e^{-\lambda t} F_{\lambda} (e^{\lambda t} \chordarcprofile'', e^{\lambda t} \chordarcprofile', e^{\lambda t} \chordarcprofile, x, t)
\end{equation}
where
\[
...
\[
e^{\lambda t} (\partial_t \psi + \lambda \psi) = \partial_t (e^{\lambda t} \psi) \geq F(e^{\lambda t} \psi'', e^{\lambda t} \psi', e^{\lambda t} \psi, x, t)
\]
at $(x_0, t_0)$, and so
\[
\begin{split}
\partial_t \psi &\geq e^{-\lambda t} F(e^{\lambda t} \psi'', e^{\lambda t} \psi', e^{\lambda t} \psi, x, t) - \lambda \psi \\
&= e^{-\lambda t} \left[F(e^{\lambda t} \psi'', e^{\lambda t} \psi', e^{\lambda t} \psi, x, t) - \lambda e^{\lambda t} \psi\right] \\
&= e^{-\lambda t} F_{\lambda}(e^{\lambda t} \psi'', e^{\lambda t} \psi', e^{\lambda t} \psi, x, t)
\end{split}
\]
...
\[
\inf_{(x, t) \in [0, 2\pi] \times [0,\tau]} \{\chordarcprofile(x, t) - \phi(x, t)\}  < 0.
\]
Let
\[
-m = \inf_{(x, t) \in [0, 2\pi] \times [0,\tau]} \{e^{-\lambda t} \chordarcprofile(x, t) - e^{-\lambda t} \phi(x, t)\} < 0
\]
and choose $(x_0, t_0)$ such that $e^{-\lambda t_0} \chordarcprofile(x_0, t_0) - e^{-\lambda t_0} \phi(x_0, t_0) = -m$. Define
\[
\psi = e^{-\lambda t} \phi - m
\]
which is a lower support function for $e^{-\lambda t} \chordarcprofile$ at $(x_0, t_0)$. The viscosity equation \eqref{eq:exponential_viscosity} for $e^{-\lambda t}\chordarcprofile$ then gives,
\[
\partial_t \psi \geq e^{-\lambda t} F_{\lambda} (e^{\lambda t} \psi'', e^{\lambda t} \psi', e^{\lambda t} \psi, x, t) = e^{-\lambda t} F_{\lambda} (\phi'', \phi', \phi - m e^{\lambda t}, x, t)
\]

On the other hand, the assumed inequality \eqref{eq:comparison_diff_ineq_leq} for $\phi$ gives,
\[
\partial_t \psi = e^{-\lambda t} (\partial_t \phi - \lambda \phi) \leq e^{-\lambda_t}(F(\phi'', \phi', \phi, x, t)  - \lambda \phi) = e^{-\lambda t} F_{\lambda}(\phi'', \phi', \phi, x, t)
\]

Putting the two previous inequalities together then yields
\begin{equation}
\label{eq:F_leq}
F_{\lambda} (\phi'', \phi', \phi - m e^{\lambda t}, x, t) \leq F_{\lambda}(\phi'', \phi', \phi, x, t)
\end{equation}
...
\[
F_{\lambda} (\phi'', \phi', \phi - m e^{\lambda t}, x, t) > F_{\lambda}(\phi'', \phi', \phi, x, t)
\]
since $\phi - m e^{\lambda t} < \phi$. Thus we have a contradiction to \eqref{eq:F_leq} and hence the theorem is true.
\end{proof}


\section{Comparison Functions}
\label{sec:orgheadline11}
Following the computations above we are looking for a function $f$ satisfying

$$4f''+\bar{Q}(f-xf')+f'(\Delta Q(p,q))-f_t\geq 0,$$

Here $\bar{Q}=\frac{1}{L}\int k^2 ds-\sum_{m=1}^n \Delta v_m$ and 
$\Delta Q(p,q)=\frac{1}{L}\int_p^q k^2 ds-\sum_{m=i_p}^{i_q} \Delta v_m
$.

The question would be how to relate the  $v_i$ with $f$ and the curve
\section{Convergence}
