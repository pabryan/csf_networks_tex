...
\[
\bar{Q} = \int_{\gamma} \curvecurv^2 ds - \sum_{m=1}^{k} \corneranglespeed_m.
\]
and
\[
...
Our comparison functions are constructed from the profile of self-simlar solutions and we collect here the basic facts about such solutions. In what follows we will need the defintions
\begin{align*}
...
\end{align*}
where $\gamma_0$ is a self-similar solution and $(p_0, q_0) \in \gamma_0$ are optimal points such that $\ell(p_0, q_0) = \frac{L}{2\pi} x$. Note that $\tfrac{L}{4\pi^2} \bar{Q}_0$ is a fixed, scale invariant number depending only on $\gamma_0$ and $\tfrac{L}{2\pi} \Delta Q_0(x)$ is a scale invariant function depending only on $\gamma_0$ that may be written in the form
\[
\tfrac{L}{2\pi} \Delta Q_0 (x) = \frac{1}{x} f_0(\psi_0'(x))
\]
for some convex function $f_0$.
