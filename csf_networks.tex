...
The main techniques we employ in this paper to understand the chord/arc-profile are variational and we begin with the first and second variations of the geometric quantities \(\ell\) and \(d\). Throughout this section we will assume that \(\gamma\) is parametrised by arc-length.

Now, we need to take a little care at a node, distinguishing between variations in the direction \(\tang^+\) and variations in the direction \(\tang^-\). For convenience, let us write \(\tang^+ = \tang^-\) for the tangent at a smooth point. Write $\omega_p^{\pm}$ and $\omega_q^{\pm}$ for the $1$-forms dual to the unit tangent vectors $\tang_p^{\pm}$ and $\tang_q^{\pm}$ at $p$ and $q$ respectively. At a smooth point \(p\), \(\omega_p^+ = \omega^-_p\) and likewise for a smooth point \(q\). Let us note that at a node, \(\omega_p^+\) acts on \(\tang_p^+\), but not on \(\tang_p^-\) so that \(\omega_p^+ (\tang_p^+) = 1\) but \(\omega_p^+ (\tang_p^-)\) is not defined. Similar considerations applying when swapping \(+\) and \(-\) and also for \(q\).

For a continuous function \(f : \gamma \times \gamma \to \RR\) that is smooth on each closed interval \([a_{i-1}, a_i] \times [a_{j-1}, a_j]\) let us write,
\[
\partial^{\pm}_p f (p_0, q_0) = \lim_{u\to 0^{\pm}} \partial_u f(p_0 + u, q_0)
\]
and
\[
\partial^{\pm}_q f (p_0, q_0) = \lim_{u\to 0^{\pm}} \partial_u f(p_0, q_0 + u)
\]
for the one-sided derivatives and define differentials,
\[
D^{\pm}_p f (p_0, q_0) = \partial^{\pm}_p f (p_0, q_0) \omega_p^{\pm}
\]
and
\[
D^{\pm}_q f (p_0, q_0) = \partial^{\pm}_q f (p_0, q_0) \omega_q^{\pm}.
\]
Again, of course, at a smooth point \(D^+_p = D^-_p\) and likewise for \(q\). Then we define the differential of \(f\) (omitting \((p_0, q_0)\) for clarity) by,
\[
\begin{split}
Df \cdot (c_p \tang_p^{\pm} + c_q \tang_q^{\pm}) &= c_p D^{\pm}_p f  (\tang_p^{\pm}) + c_q D^{\pm}_q f  (\tang_q^{\pm}) \\
&= c_p \partial^{\pm}_p f  + c_q \partial^{\pm}_q f
\end{split}
\]
written more briefly as,
\[
Df = \partial^{\pm}_p f \omega^{\pm}_p + \partial^{\pm}_q f \omega^{\pm}_q.
\]

Note that the \(\pm\) on the \(p\) is independent of the \(\pm\) on the \(q\) so for example,
\[
Df \cdot(\tang_p^+ + \tang_q^-) = \partial_p^+ f + \partial_q^- f
\]
is a legitimate expression.

For the reader's convenience, let us recall from Definition \ref{defn:dist} that the extrinsic distance is defined to be,
\[
d_t(p, q) = \|\gamma_t(q) - \gamma_t(p)\|.
\]
...
\ell_t(p, q) = \int_{\gamma_t} ds_t = \int_p^{a_i} ds_t + \sum_{m=i+1}^{j-1} \int_{a_{m-1}}^{a_m} ds_t + \int_{a_{j-1}}^q ds_t.
\]
We also denoted by \(w\) in equation \eqref{eq:w}, the unit vector pointing from \(\gamma(p)\) to \(\gamma(q)\).

\begin{lemma}[First Variation]
\label{lem:firstvar}
\[
D\ell = -\omega_p^{\pm} + \omega_q^{\pm}
\]
and
\[
D d = -\ip{\tang^{\pm}_p}{w} \omega_p^{\pm} + \ip{\tang^{\pm}_q}{w} \omega_q^{\pm}.
\]
\end{lemma}

\begin{proof}
The variations of \(l\) follow by taking one-sided derivatives of \(\int_{p + u}^{a_i} ds_t\) and \(\int_{a_{j-1}}^{q + u} ds_t\) with respect to u.

For \(d\),
\[
\begin{split}
\lim_{u\to 0^{\pm}} \partial_u \ip{\gamma(q) - \gamma(p + u)}{\gamma(q) - \gamma(p + u)} &= \lim_{u\to 0^{\pm}} \left(- 2 \ip{\tang(p+u)}{\gamma(q) - \gamma(p+u)}\right) \\
&= - 2 \ip{\tang^{\pm}(p)}{\gamma(q) - \gamma(p)}.
\end{split}
\]
Hence,
\[
\lim_{u\to 0^{\pm}}  \partial_u d (p + u, q) =  -\frac{1}{d} \ip{\tang^{\pm}(p)}{\gamma(q) - \gamma(p)} = -\ip{\tang^{\pm}(p)}{w}.
\]
The variation in the second argument is similar.
\end{proof}

Next we have the second variation. As with first derivatives, we can take one-sided second derivatives such as,
\[
\partial_p^+ \partial_p^+ f = \lim_{u \to 0^+} \partial_u^2 f(p_0 + u, q_0),
\]
but expressions like,
\[
\partial_p^+ \partial_p^- f
\]
are not allowed since \(\partial_p^- f (p_0 + u, q_0)\) is not defined for \(u > 0\), hence preventing us from applying \(\partial_p^+\). On the other hand, for the mixed derivatives, we are allowed to mix up the signs by defining,
\[
\partial_p^{\pm} \partial_q^{\pm} f = \partial_u^{\pm} \partial_v^{\pm} f(p_0 + u, q_0 + v)
\]
where the \(\pm\) on the \(p\) and the \(\pm\) on the \(q\) may vary independently. From the point of view of the differentials, \(\omega_p^{\pm}\) and \(\omega_q^{\pm}\), this implies we are allowed to form expressions such as
\[
\omega_p^+ \otimes \omega_q^-,
\]
but not expressions such as
\[
\omega_p^+ \otimes \omega_p^-.
\]
More generally an expression like,
\[
\omega_p^+ \otimes \omega_p^+ + \omega_p^+ \otimes \omega_q^- + \omega_q^- \otimes \omega_q^-
\]
is allowed, but
\[
\omega_p^+ \otimes \omega_p^+ + \omega_p^- \otimes \omega_q^- + \omega_q^- \otimes \omega_q^-
\]
is not allowed. In the first example, the given two-form acts on \(\tang_p^+ \oplus \tang_q^-\).  In the second example the first term can act on \(\tang_p^+ \oplus \tang_q^{\pm}\), the second term acts on \(\tang_p^- \oplus \tang_q^-\) and the final term can act on \(\tang_p^{\pm} \oplus \tang_q^-\). In all cases we can define, \(\omega_p^+(\tang_q^{\pm}) = 0\) and likewise swapping \(p\) and \(q\). Thus there is no problem allowing \(\omega_p^+ \otimes \omega_p^+\) to act on \(\tang_p^+ \oplus \tang_q^{\pm}\). But in the second example, there is a problem with the first term acting on \(\tang_p^+\) while the second term acts on \(\tang_p^-\). None of this causes any concern about our applications however, since the first example arises through the one-sided variation \(u \geq 0 \mapsto (p_0, q_0) + u(1, -1)\) while the second example can never arise through a one-sided variation.

So with all this in mind, let us define,
\[
\begin{split}
D^2 f &= \partial_p^{\pm} \partial_p^{\pm} f (\omega_p^{\pm} \otimes \omega_p^{\pm}) + \partial_q^{\pm} \partial_q^{\pm} f (\omega_q^{\pm} \otimes \omega_q^{\pm}) \\
&\quad + \partial_p^{\pm} \partial_q^{\pm} f (\omega_p^{\pm} \otimes \omega_q^{\pm}) + \partial_q^{\pm} \partial_p^{\pm} f (\omega_q^{\pm} \otimes \omega_p^{\pm})
\end{split}
\]
with the understanding that all of the \(\pm\) attached to \(p\)'s are the same, and all of the \(\pm\) attached to \(q\)'s are the same, which rules out the second example above. For ease of reading, we will omit the \(\pm\) writing simply,
\[
\begin{split}
D^2 f &= \partial_p \partial_p f (\omega_p \otimes \omega_p) + \partial_q \partial_q f (\omega_q \otimes \omega_q) \\
&\quad + \partial_p \partial_q f (\omega_p \otimes \omega_q) + \partial_q \partial_p f (\omega_q \otimes \omega_p)
\end{split}
\]
where it is understood that all \(p\)-terms are adorned with a \(+\) or all \(p\)-terms are adorned with a \(-\) (but never with \(\pm\) mixed). Similarly all \(q\)-terms are adorned with the same \(\pm\). The \(p\) and \(q\) adornments are however independent, so for example, all \(p\)'s may be adorned with a \(+\) and all \(q\)'s may be adorned with a \(-\).
...
\end{split}
\]
These second derivatives yield the desired expression for \(D^2 d\) with the appropriate \(\pm\) terms coming from taking one-sided derivatives.
\end{proof}

Using Lemmas \ref{lem:firstvar} and \ref{lem:secondvar} we may now obtain the second variation formula at optimal points. We begin with smooth points for which we not burdened with keeping track of the various \(\pm\) and the derivation is much cleaner. With the smooth case in hand, then we give the case of nodes which should be then be reasonably clear. So, for now, we assume \((p,q) \in \gamma^{\ast} \otimes \gamma^{\ast}\) is a smooth point.

We first introduce some notation. Write \(\theta(p,q) \in [0, \pi)\) for the angle between \(\tang_{p}\) and \(w\) so that \(\ip{w}{\tang_p} = \cos\theta\). Define the following "indicator" functions,
\begin{align*}
...
\label{prop:optimal_smooth_var}
Let $(p_0, q_0) \in \gamma{\ast} \times \gamma^{\ast}$ be a smooth point, such that $d(p_0, q_0)$ is minimum amongst all $(p,q)$ with $\ell(p, q) = \ell(p_0, q_0)$ (or more briefly, $\chordarcprofile\left(\tfrac{L}{2\pi}\ell(p_0,q_0)\right) = \tfrac{2\pi}{L} d(p_0, q_0)$). Then
\[
\begin{split}
...
Now let us consider the case of a node. Define \(\theta_p^{\pm} \in [0, \pi)\) by \(\cos\theta_p^{\pm} = \ip{w}{\tang_p^{\pm}}\) and likewise for \(\theta_q^{\pm}\). Since \(\ip{\tang_p^+}{\tang_p^-} = \tfrac{1}{2}\), \(\theta_p^+\) and \(\theta_p^-\) are related, but this relation depends on how \(w\) relates to \(\tang_p^{\pm}\). For example, if \(w\) lies between \(\tang_p^+\) and \(\tang_p^-\), then \(\theta_p^- = \tfrac{\pi}{3} - \theta_p^+\). The remaining possible configurations are \(\theta_p^- = \tfrac{\pi}{3} + \theta_p^+\), \(\theta_p^- = \tfrac{5\pi}{3} - \theta_p^+\), and \(\theta_p^- = \theta_p^+ - \tfrac{\pi}{3}\).

The main difference with the smooth case is that the length preserving variation \(\alpha(u) = (p_0, q_0) + uv_1\) needs to be one-sided. This leads to two first variation identities - one for each side - identifying \(\cos\theta_p^{\pm} = \cos \theta_q^{\pm}\) with the \(\pm\) preserved all the way through. Then everything else proceeds in much the same way as for the smooth case. But be aware that now we can take \(\omega_p^{\pm}\) and \(\omega_q^{\pm}\) with independent \(\pm\)'s. Thus for instance,
\[
D d (\tang_p^+ \oplus \tang_q^+) = (-\cos\theta_p^+ \omega_p^+ + \cos\theta_q^+ \omega_q^+) (\tang_p^+ \oplus \tang_q^+) = 0
\]
since \(\theta_q^+ = \theta_q^+\), while
\[
\begin{split}
D d (\tang_p^+ \oplus \tang_q^-) &= (-\cos\theta^+\omega_p^+ + \cos\theta^-\omega_q^-) (\tang_p^+ \oplus \tang_q^-) \\
&= \begin{cases}
0 & \text{at at smooth point}, \\
-\cos\theta^+ + \cos\theta^- \ne 0 & \text{ at a node}.
\end{cases}
\end{split}
\]

Now let us introduce the appropriate notions of \(K\) and \(M\) where \(p\) or \(q\) (or both) are allowed to be nodes. First we define the indicator functions as before.

\[
\tangindicator^{\pm} (p, q) = \begin{cases}
1,& \tang_{p}^{\pm} \ne \tang_{q}^{\pm} \\
-1,& \tang_{p}^{\pm} = \tang_{q}^{\pm}
\end{cases}
\]
keeping in mind that \(\tang_p^+ = \tang_q^+\) if and only if \(\tang_p^- = \tang_q^-\) so that \(\tangindicator^+ = \tangindicator^-\) and we may drop the \(\pm\) from \(\tangindicator\). The other indicator function is,
\[
\norindicator^{\pm} (p, q) = \begin{cases}
1,& \ip{w}{\nor_{p}^{\pm}} > 0 \\
-1,& \ip{w}{\nor_{p}^{\pm}} < 0.
\end{cases}
\]

Since we need to deal with terms like \(\omega_p^+ \otimes \omega_q^-\) where the signs are different, it's helpful to adorn the indicator functions with \(p\) or \(q\) and so write expressions such as,
\[
\tangindicator_p^{\pm} \omega_p^{\pm} \otimes \omega_p^{\pm} + \tangindicator_q^{\pm} \omega_q^{\pm} \otimes \omega_q^{\pm}
\]
where it's understood that all the terms adorned with \(p\) have the same sign, and all the terms adorned with \(q\) have the same sign. Thus for example, the expression,
\[
\tangindicator_p^+ \omega_p^+ \otimes \omega_p^+ + \tangindicator_q^- \omega_q^- \otimes \omega_q^-
\]
is allowed.

\begin{prop}[Spatial Variations at Arbitrary Points]
\label{prop:optimal_var}
Let $(p_0, q_0) \in \gamma \times \gamma$ be an arbitrary point such that $d(p_0, q_0)$ is minimum amongst all $(p,q)$ such that $\ell(p, q) = \ell(p_0, q_0)$. Then
\[
\begin{split}
D d &= -\cos\theta_{p_0}^{\pm}\omega_{p_0}^{\pm} + \cos\theta_{q_0}^{\pm} \omega_{q_0}^{\pm} \\
\intertext{and}
D^2d_{(p_0,q_0)} &= \sqrt{1-\cos^2\theta_{p_0}^{\pm}}\norindicator_p^{\pm} \curvecurv_p^{\pm} \omega_p^{\pm} \otimes \omega_p^{\pm} + \sqrt{1-\cos^2\theta_{q_0}^{\pm}} \norindicator_q^{\pm} \curvecurv_q^{\pm} \omega_q^{\pm} \otimes \omega_q^{\pm} \\
&\quad + \frac{1-\cos^2 \theta^{\pm}}{d} M.
\end{split}
\]
\end{prop}

\begin{proof}
As in the smooth case, defining \(\alpha(u) = (p_0, q_0) + uv_1\) yields \(\ell(\alpha(u)) = \ell(\alpha(0))\) is constant. Taking one-sided derivatives yields,
\[
\cos\theta_p^{\pm} = \ip{w}{\tang_p^{\pm}} = \ip{w}{\tang_q^{\pm}} = \cos\theta_q^{\pm}
\]
Similarly to the smooth case,
\[
\sqrt{1 - \cos^2 \theta_p^{\pm}} = \norindicator^{\pm}_p \ip{w}{\nor_p^{\pm}}
\]
and,
\[
\sqrt{1 - \cos^2 \theta_q^{\pm}} = \norindicator^{\pm}_q \ip{w}{\nor_q^{\pm}}.
\]
...
Next, let us note that for strictly convex \(\gamma\), optimal points are never nodes.

\begin{lemma}
Let \(\gamma\) be strictly convex and let \((p_0,q_0) \in \sphere^1 \times \sphere^1\) be optimal; that is \(Z(\tfrac{2\pi}{L}\ell(p_0,q_0)) = \tfrac{L}{2\pi} d(p_0, q_0)\). Then \(p_0,q_0 \notin \{a_i\}_{i=1}^k\). That is, neither \(p_0\) nor \(q_0\) is a node.
\end{lemma}

\begin{proof}
We show that if at least one of \(p_0, q_0\) is a node, then there is a length preserving variation strictly decreasing \(d\) and hence \((p_0, q_0)\) cannot be optimum.

The argument is symmetric in \(p_0, q_0\) so without loss of generality, suppose \(p_0 = a_i\) is a node and \(q_0\) is any other point, possibly also a node. Let \(\alpha_{\pm} (u) = (a_i \pm u, q_0 \pm u)\). Then \(\ell(\alpha_{\pm} (u)) = \ell(a_i, q_0)\) is constant. Lemma \ref{lem:firstvar} gives
\[
\left.\partial_u\right|_{u=0} d (\alpha_{\pm} (u)) = \mp\ip{\tang^{\pm}(a_i)}{w} \pm \ip{\tang^{\pm} (q_0)}{w}.
\]

First consider the case where \(q_0\) is not a node, in which case \(\tang^-(q_0) = \tang^+(q_0)\). If \(-\ip{\tang^+(a_i)}{w} + \ip{\tang^+ (q_0)}{w} < 0\), then we are done so suppose that \(-\ip{\tang^+(a_i)}{w} + \ip{\tang^+ (q_0)}{w} \geq 0\). Then
\[
\ip{\tang^-(a_i)}{w} - \ip{\tang^- (q_0)}{w} \leq \ip{\tang^-(a_i) - \tang^+(a_i)}{w}.
\]
\end{proof}
