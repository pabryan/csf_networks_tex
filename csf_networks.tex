...
Now we may express the first and second variations of distance in this notation. The derivation is quite standard these days, but we give the full details here to be complete, and to illustrate the effect of the nodes. The opposite situation the time variations holds, in that the presense of the nodes felt by the extrinsic distance and not the intrinsic distance. Recall from Definition \ref{defn:dist} that the extrinsic distance is defined to be,
\[
d_t(p, q) = \|\gamma_t(q) - \gamma_t(p)\|.
\]
...
We need to take a little care at a node, distinguishing between variations in the direction \(\tang^+\) and variations in the direction \(\tang^-\). It's easier to phrase this by varying \(p\) and \(q\) individually and then using linearity. For convenience, let us also write \(\tang^+ = \tang^-\) for the tangent at a smooth point. Throughout we assume an arc-length parametrisation.

\begin{lemma}
\label{lem:firstvar}
The first variation formulae are
\[
\begin{split}
