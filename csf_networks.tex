...
The derivation of the following variation formula is quite standard but we give the full details here to be complete. Recall from Definition \ref{defn:dist} that the extrinsic distance is defined to be,
\[
d_t(p, q) = \|\gamma_t(q) - \gamma_t(p)\|.
\]
...
We need to take a little care at a node, where only one-sided variations are defined. It seems simplest to vary \(p\) and \(q\) individually and then using linearity. For convenience, let us also write \(\tang^+ = \tang^-\) for the tangent at a smooth point.
...
\left.\partial_u\right|_{u=0} \ell (p, q \pm u) &= \pm 1.
\end{split}
\]
and
\[
...
Let $\varphi_0 : [0, 2\pi] \times \RR_{\geq 0} \to \RR$ be a smooth function satisfying:
\begin{itemize}
