...
We may now deduce the following comparison theorem:

\begin{theorem}
\label{thm:comparison}

Let $\varphi : [0, 2\pi] \times \RR_{\geq 0} \to \RR$ be a continuous function satisfying:
\begin{itemize}
\item $\varphi$ is smooth and positive on $(0, 2\pi) \times \RR_{>0}$,
\item $\varphi$ symmetric about $x = \pi$ for every $t$,
\item $\varphi(0, t) = \varphi(2\pi, t) = 0$ for every $t$,
\item $\varphi$ is weakly concave in $x$ for each $t$ ($\varphi'' \leq 0)$.
\end{itemize}

If
\[
\partial_t \varphi \leq \left(\frac{2\pi}{L}\right)^2 4 \varphi'' + \frac{1}{L} \bar{Q} (\varphi - x \varphi') + \frac{2\pi}{L} \Delta Q \varphi'.
\]
and $\varphi(x, 0) \leq \chordarcprofile(x, 0)$ for all $x \in [0, 2\pi]$, then $\varphi(x, t) \leq \chordarcprofile(x, t)$ for all $x, t$.

If in addition,
\[
\varphi(x, t) = \frac{\sqrt{3}}{2} x - c(t) x^2 + \bigo(x^3) \quad \text{as} \quad x \to 0
\]
then
\[
\frac{L}{2\pi} \max_i \{\curvecurv^+(a_i, t) + \curvecurv^-(a_i, t)\} \leq 16 c(t).
\]
for all $t$.
\end{theorem}
...
\]
so that the assumptions on $\varphi$ give
\begin{equation}
...
From Theorem \ref{thm:comparison}, we want to find positive, symmetric about $\pi$, concave functions $\varphi_c$ such that
\[
\partial_t \varphi_c \leq \left(\frac{2\pi}{L}\right)^2 4 \varphi_c'' + \frac{1}{L} \bar{Q} (\varphi_c - x \varphi_c') + \frac{2\pi}{L} \Delta Q \varphi_c'.
\]
We want that a choice of parameter $c$ gives $\varphi_c(x, 0) \leq \chordarcprofile(x, 0)$ - something like $\varphi_c \to 0$ as $c\to\infty$. We also require the asymptotics,
\[
\varphi(x, t) = \frac{\sqrt{3}}{2} x - c(t) x^2 + \bigo(x^3) \quad \text{as} \quad x \to 0.
\]

The question would be how to relate the $v_i$ with $\varphi$ and the curve.

\subsection{Comparison for the normalised flow}

Working with the normalised flow instead we have:

$$4f''+\bar{Q}(f-xf')+f'(\Delta Q(p,q))-f_t\geq 0,$$

Here $\bar{Q}=\frac{1}{2\pi}\int k^2 ds-\sum_{m=1}^n \Delta v_m$ and $\Delta Q(p,q)=\frac{1}{2\pi}\int_p^q k^2 ds-\sum_{m=i_p}^{i_q} \Delta v_m$.

\textcolor{red}{Should there also be $1/2\pi$ attached to the $v_m$ terms?}
